...
export default function SidebarDashboard({ activeTab, onTabChange, className = '', darkMode = false }) {
  ...
  return (
    <aside className={`w-64 border-r min-h-screen p-4 hidden md:flex flex-col gap-6 transition-colors duration-300 ${className} ${darkMode
      ...
      <div className="flex items-center gap-3 px-2">
        ...
        <div>
          <div className={`text-sm font-bold tracking-wide ${darkMode ? 'text-white' : 'text-slate-900'}`}>SUTHRA</div>
              ...
              <li key={item.id}>
                ...
                >
                  {isActive && (
                    <motion.div
                      layoutId="activeTab"
                      className={`absolute inset-0 border ${darkMode
                          ? 'bg-gradient-to-r from-sky-500/10 to-indigo-500/10 border-sky-500/20'
                          : 'bg-gradient-to-r from-sky-500 to-indigo-600 border-transparent'
                        }`}
                      initial={false}
      ...
      <div className="px-4 py-4 mt-auto">
        <div className={`rounded-2xl p-4 border relative overflow-hidden group transition-colors duration-300 ${darkMode
          ...
          <div className={`text-[10px] font-bold uppercase tracking-wider mb-1 ${darkMode ? 'text-indigo-400' : 'text-indigo-600'}`}>System Status</div>
